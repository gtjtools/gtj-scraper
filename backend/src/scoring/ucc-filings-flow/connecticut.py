...
class ConnecticutFlow(BaseUCCFlow):
    ...
    async def fill_search_form(self, page: Page, search_query: str) -> bool:
        ...
        Steps:
        1. Look for UCC/Lien search option
        2. Select organization debtor type if needed
        3. Fill in the organization name field
        4. Click the search button
        5. Wait for results
        """
        try:
            print(f"📝 Filling Connecticut UCC search form for: {search_query}")
            ...
            await self.take_screenshot(page, f"connecticut_search_results.png")

            print("✓ Connecticut search form completed successfully")
            return True
        except Exception as e:
            print(f"❌ Connecticut form fill error: {str(e)}")
            await self.take_screenshot(page, f"connecticut_error.png")
            return False
    ...
    async def extract_results(self, page: Page) -> Dict[str, Any]:
        ...
        """
        try:
            print("📊 Extracting Connecticut UCC search results...")
            ...
            await self.take_screenshot(page, f"connecticut_final_results.png")
            ...
            print("✓ Connecticut results extraction completed")
