...
class IowaFlow(BaseUCCFlow):
    ...
    async def fill_search_form(self, page: Page, search_query: str) -> bool:
        ...
        Steps:
        1. Look for organization/business name input
        2. Fill in the organization name field
        3. Click the search button
        4. Wait for results
        """
        try:
            print(f"📝 Filling Iowa UCC search form for: {search_query}")

            # Step 1: Look for organization name input
            print("   Step 1: Looking for organization/business name input...")
            input_selectors = [
                'input[name*="business"]',
                'input[name*="Business"]',
            ...
            await self.take_screenshot(page, f"iowa_search_results.png")

            print("✓ Iowa search form completed successfully")
            return True
        except Exception as e:
            print(f"❌ Iowa form fill error: {str(e)}")
            await self.take_screenshot(page, f"iowa_error.png")
            return False
    ...
    async def extract_results(self, page: Page) -> Dict[str, Any]:
        ...
        """
        try:
            print("📊 Extracting Iowa UCC search results...")
            ...
            await self.take_screenshot(page, f"iowa_final_results.png")
            ...
            print("✓ Iowa results extraction completed")
            ...
            return {
                ...
                "search_cost": "Free (certified searches available)",
                "notes": f"Extracted {len(filings)} UCC filing records. Free certified lien searches available.",
            }
        except Exception as e:
